...
const Comment = ({ postid }) => {
    ...
    const makeComment = () => {
        ...
        fetch('https://62983daaf2decf5bb73ddb37.mockapi.io/comment', {
            ...
            .then((result) => {
                ...
                let cmt=JSON.parse(comment)
                let anc = cmt.concat(result)
                console.log((anc));
                localStorage.removeItem("comment")
                localStorage.setItem("comment",JSON.stringify(anc))
        ...
        try{
            fetch(`https://62983daaf2decf5bb73ddb37.mockapi.io/comment/${id}`, {
                ...
                .then((result) => {
                    ...
                    let comment=localStorage.getItem('comment')
                   let cmtdata=(JSON.parse(comment))
                    console.log(cmtdata);
                    let anc = cmtdata?.filter((item) => item.id !== id)
                    console.log(anc);
                    localStorage.setItem("comment",JSON.stringify(anc))
                })
        }
    ...
    const UpdateComment = (id) => {
        ...
        fetch(`https://62983daaf2decf5bb73ddb37.mockapi.io/comment/${id}`, {
            ...
            .then((result) => {
                ...
                let comment=localStorage.getItem('comment')
                let cmtdata=(JSON.parse(comment))
                const pindex = cmtdata.findIndex(c => c.id === id);
                cmtdata[pindex] = result;
                console.log("Data==",cmtdata);
                localStorage.setItem("comment",JSON.stringify(cmtdata))
                setComment("")
            })
    }
    ...
    return (
        <div>
            ...
            {
                commentsdata?.map((cd,i) => (
                    // console.log(cd.postid===postid),
                    cd?.postid === postid &&

                    <Box sx={{ display: 'flex', m: 1 }} key={cd.id + i}>
                        <CardContent>
